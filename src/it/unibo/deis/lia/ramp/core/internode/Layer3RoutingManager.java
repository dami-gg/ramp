...
public class Layer3RoutingManager implements PacketForwardingListener{
    ...
    private static void setLayer3NextHop(String from, String to) throws Exception{
        ...
        if(RampEntryPoint.os.startsWith("windows")){
            try{
                if(from!=null){
                    // 1) enable NAT
                    /*String com1 = "net stop SharedAccess";
                    ...
                    String com4 = pre + "uninstall";
                    System.out.println("Layer3RoutingManager "+com4);
                    Process p4 = Runtime.getRuntime().exec(com4);
                    p4.waitFor(); p4.destroy();

                    String com5 = pre + "install";
                    System.out.println("Layer3RoutingManager "+com5);
                    Process p5 = Runtime.getRuntime().exec(com5);
                    p5.waitFor(); p5.destroy();

                    String com6 = pre + "set global tcptimeoutmins=1440 udptimeoutmins=1 loglevel=ERROR";
                    System.out.println("Layer3RoutingManager "+com6);
                    Process p6 = Runtime.getRuntime().exec(com6);
                    p6.waitFor(); p6.destroy();*/

                    /*if(to!=null){
                        String interfTo = Layer3RoutingManager.fromIpToName(to.getHostAddress());
                        String com7 = pre + "add interface name=\""+interfTo+"\" private";
                        System.out.println("Layer3RoutingManager "+com7);
                        Process p7 = Runtime.getRuntime().exec(com7);
                        p7.waitFor(); p7.destroy();
                    }
                    else{
                        // String com8 = pre + "add interface name=\""+interfDefaultGateway+"\" private";
                    }*/

                    /*String interfFrom = Layer3RoutingManager.fromIpToName(from.getHostAddress());
                    String com8 = pre + "add interface name=\""+interfFrom+"\" full";
                    System.out.println("Layer3RoutingManager "+com8);
                    Process p8 = Runtime.getRuntime().exec(com8);
                    p8.waitFor(); p8.destroy();*/
                ...
                /*if(to!=null){
                    String interfTo = Layer3RoutingManager.fromIpToName(to.getHostAddress());
                    String comDNS = "netsh routing ip add dns \""+interfTo+"\" static 137.204.58.1";
                    System.out.println("Layer3RoutingManager "+comDNS);
                    Process pDNS = Runtime.getRuntime().exec(comDNS);
                    pDNS.waitFor(); pDNS.destroy();
                    comDNS = "netsh routing ip add dns \""+interfTo+"\" static 137.204.59.1";
                    System.out.println("Layer3RoutingManager "+comDNS);
            ...
            try{
                ...
                writerDns.newLine();
                writerDns.flush(); writerDns.close();
    ...
    private static String sudoCommand(String command) throws Exception{
        ...
        String[] commandArray = {
            "sh", "-c", "sudo -S "+command+" 2>&1"
        };
        /*System.out.print("sudoCommand.commandArray: ");
        ...
        /*System.out.print("sudoCommand.commandK: ");
        for(String s : commandK){
            System.out.print(s+" ");
        }
        System.out.println();/**/
        ...
        /*BufferedReader brK = new BufferedReader(new InputStreamReader(pK.getInputStream()));
        while( (line=brK.readLine()) != null ){
            System.out.println("sudoCommand line k: "+line);
        }/**/
    ...
    private static String getSuperuserPassword() {
        ...
        if(superuserPassword==null){
            ...
            int res = javax.swing.JOptionPane.showConfirmDialog(
                    null, message, "Granting root privileges", javax.swing.JOptionPane.OK_CANCEL_OPTION, javax.swing.JOptionPane.QUESTION_MESSAGE
            );
